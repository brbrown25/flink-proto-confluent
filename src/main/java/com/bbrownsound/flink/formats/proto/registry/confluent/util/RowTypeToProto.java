 ...
 * com.google.protobuf.Descriptors.Descriptor}. Derived from
...
public class RowTypeToProto {
   ...
   *
   * @param logicalType the Flink row type
   * @param rowName the name for the top-level message type
   * @param packageName the Protobuf package name
   * @return a descriptor for the equivalent Protobuf message type
   */
  public static Descriptor fromRowType(RowType logicalType, String rowName, String packageName) {
    try {
  ...
  private static DescriptorProto fromRowType(
    ...
    for (int i = 0; i < logicalType.getFieldCount(); i++) {
      ...
      builder.addField(
          fromRowField(field.getType(), field.getName(), i + 1, nestedRows, dependencies, false));
    }
        ...
        builder.setOptions(
            FieldOptions.newBuilder()
                .setExtension(
                    ...
                    Meta.newBuilder()
                        .putParams(
                            CommonConstants.CONNECT_TYPE_PROP, CommonConstants.CONNECT_TYPE_INT8)
                        .build())
                .build());
        return builder.build();
      case SMALLINT:
        ...
        builder.setOptions(
            FieldOptions.newBuilder()
                .setExtension(
                    ...
                    Meta.newBuilder()
                        .putParams(
                            CommonConstants.CONNECT_TYPE_PROP, CommonConstants.CONNECT_TYPE_INT16)
                        .build())
                .build());
        return builder.build();
      case INTEGER:
